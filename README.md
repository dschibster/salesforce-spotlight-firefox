...
**2.1.0** (Jul 2026). The authoritative version is in `manifest.json`.

- **2.1.0** - **Custom Settings** search: direct **Open** + **Manage Records** links (like CMDT), with a `/cs` slash command and a settings toggle. **User-defined setup pages**: add any Setup page (paste its URL, a `/lightning/setup/…` path, or just the node name) in the popup to include it in search — no hardcoding. Permission Set Groups no longer surface their auto-created "group" permission set, which had no openable page. Metadata cache retention raised from 10 to 60 minutes and keyed per **org** (canonical `*.my.salesforce.com`), so Lightning views and the Setup domain share one cache. Added a **Users (Classic list)** setup entry alongside the Lightning one, so both work regardless of the org's Enhanced User List View setting; opening one makes it stick per org (only the picked variant shows) until the cache is flushed.
- **2.0.1** - Bar auto-collapses if the search input sits unfocused for 8 seconds (click away or switch tabs); refocusing before then cancels it.
